...
func setupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "root@tcp(localhost:3306)/learn-golang-restful-pzn-test")
...
func TestUpdateCategorySuccess(t *testing.T) {
	...
	requestBody := strings.NewReader(`{"name": "GADGET2"}`)
...
func TestGetListCategorySuccess(t *testing.T) {
	...
	assert.Equal(t, "Gadget", categoryResponse1["name"])

	assert.Equal(t, category2.Id, int(categoryResponse2["id"].(float64)))
	assert.Equal(t, "Computer", categoryResponse2["name"])
}
